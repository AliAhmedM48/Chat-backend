...
export default class ChatMongoRepository {
  ...
  create = async (users: string[], isGroup: boolean, name: string, lastMessage?: string) => {
  ...
  findDirectChatBetweenUsers = async (loggedUser_id: string, receiverId: string) => {
  ...
  update = async (chatId: string, update: { users?: string[], name?: string, isGroup?: boolean, lastMessage?: string }) => {
    return await Chat.findByIdAndUpdate(chatId, update, { new: true });
  }
